...
const EditSale = () => {
    ...
    const loadSaleData = async () => {
      try {
        ...
        if (result.success) {
          ...
          // Set form data from existing sale
          setFormData({
            propertyId: sale.property_id || "",
            propertyType: sale.property_type || "",
            propertyTitle: sale.property_title || "",
            propertyAddress: sale.property_address || "",
            propertyCity: sale.property_city || "",
            propertyGovernorate: sale.property_governorate || "",
            surfaceArea: sale.surface_area || "",
            numberOfRooms: sale.number_of_rooms || "",
            bathrooms: sale.bathrooms || "",
            mainPrice: sale.main_price || "",
            ownerName: sale.owner_name || "",
            ownerPhone: sale.owner_phone || "",
            clientId: sale.client_id || "",
            clientName: sale.client_name || "",
            clientPhone: sale.client_phone || "",
            clientEmail: sale.client_email || "",
            buyType: sale.buy_type || "cash",
            clientNote: sale.client_note || "",
            saleStatus: sale.sale_status || "promise",
            signatureDate: sale.signature_date
              ? sale.signature_date.split("T")[0]
              : "",
            percentagePaid: sale.percentage_paid || "",
            amountPaid: sale.amount_paid || "",
            remainingSaleAmount: sale.remaining_sale_amount || "",
            saleNote: sale.sale_note || "",
            senateurState: sale.senateur_state || "preparation",
            propertyNotes: sale.property_notes || "",
            agentName: sale.agent_name || "",
            agentCommission: sale.agent_commission_percentage || "",
            commissionPaid: sale.commission_paid || false,
          });

          // Set search fields
          if (sale.property_title) {
            setPropertySearch(
              `${sale.property_title} - ${sale.property_city || ""}`
            );
            setSelectedProperty({
              property_title_overview: sale.property_title,
            });
          }

          if (sale.client_name) {
            setClientSearch(`${sale.client_name} - ${sale.client_phone || ""}`);
            setSelectedClient({ full_name: sale.client_name });
          }
        } else {
          throw new Error(result.message || "Failed to load sale data");
        }
      } catch (error) {
        console.error("Error loading sale:", error);
        if (window.showToast) {
          window.showToast(`Failed to load sale: ${error.message}`, "error");
        }
        navigate("/sales");
      } finally {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/properties/search",
        {
          ...
          body: JSON.stringify({
            query: searchTerm,
            excludeSold: true, // Exclude sold properties
          }),
        }
      );
  ...
  const searchClients = async (searchTerm) => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.success) {
        setClientResults(result.data.slice(0, 10));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (window.showToast) {
        window.showToast(`Failed to update sale: ${error.message}`, "error");
      }
    } finally {
      setLoading(false);
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleInputChange("buyType", e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
              ...
              <p className="text-sm text-yellow-700">
                You can update the amount paid to add new payments or modify
                existing ones.
              </p>
            </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-16 flex items-center justify-center">
        <div className="text-center">
          ...
          <span className="block mt-2 text-gray-600">Loading sale data...</span>
        </div>
      </div>
    );
  }
